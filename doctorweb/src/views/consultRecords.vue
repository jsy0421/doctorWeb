<template>
	<div>
		...
		<Head :avatarUrl="avatarUrl" :doctorName="doctorName" :doctorPosition="doctorPosition" :currentPage=2></Head>
			...
			<div class="content">
				<div class="container"
					...
					<div class="container-header"
						:class="{ go: rotateIndex == index ? rotate :'', aa: rotateIndex == index ? !rotate: '' }">
						完成时间：{{ item.finishTime.split('T')[0] }} {{ item.finishTime.split('T')[1].split('.')[0] }}
					</div>
                  ...
                  ite
					...
					<div class='container-footer'
						:class="{ go: rotateIndex == index ? rotate :'', aa: rotateIndex == index ? !rotate: '' }">
						<button @click="lookPrescription(index)" class="footer-button">查看处方</button>
					</div>
				</div>
			</div>
			<div class="footers">
		...
		<div v-else>
			<el-empty :image-size="200" description="暂无接诊记录"></el-empty>
		</div>
	</div>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			lookPrescription(index) {
				console.log('look')
				this.$router.replace({
					path: '/detail',
					query: this.containerlist[index]
				})
			},
			// 获取数据
			getData() {
